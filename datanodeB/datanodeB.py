...
@app.route("/ping")
def ping():
    return jsonify({"status": "datanodeB-ok", "time": time.time()})
...
if __name__ == "__main__":
    ...
    register()
    print(f"[DATANODE B] Running on {config['datanodeB']['host']}:{config['datanodeB']['port']}")
    app.run(host=config["datanodeB"]["host"], port=config["datanodeB"]["port"])
